...
uint64_t LBAwrite(void *buffer, uint64_t lbaCount, uint64_t lbaPosition)
{
	...
	if (lbaPosition > partInfop->numberOfBlocks) // Not a valid start position
		return 0;
	...
	if (fl.l_start < partInfop->blocksize) // not a valid start position
		return 0;
...
uint64_t LBAread(void *buffer, uint64_t lbaCount, uint64_t lbaPosition)
{
	...
	lseek(partInfop->fd, fl.l_start, SEEK_SET);
	retRead = read(partInfop->fd, buffer, fl.l_len);
...
void runFSLowTest()
	...
	if (partInfop == NULL)
		{
		printf("System not initialized.  Test Failed");	
		return;
		}

	buf = malloc(partInfop->blocksize);
    if (buf == NULL)
		{
		printf("Failed to malloc initial buffer.  Test Failed");
		return;
		}

	for (int i = -2; i < 128; i++)
		{
		memset(buf, i, partInfop->blocksize);
		result = LBAwrite (buf, 1, i);
		printf ("Wrote block %d with a result of %d\n", i, result);
		result = LBAread (buf, 1, i);
		printf ("Read block %d with a result of %d\n", i, result);
		}
